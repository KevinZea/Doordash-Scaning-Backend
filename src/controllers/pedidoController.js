...
export async function updatePedido(req, res) {
    try {
        ...
        const { name, pedido, restaurant } = req.body; // Los nuevos datos del restaurante
        const data = await fs.readFile(dataPath, 'utf8');
        let dataName = JSON.parse(data)
        ...
        if (index !== -1) {
            ...
            dataName[index] = { ...dataName[index], name, pedido, restaurant };
...
export async function updatePedidoStatus(req, res) {
    try {
        const { id } = req.params; // El ID del restaurante que quieres editar
        const { status } = req.body; // Los nuevos datos del restaurante
        const data = await fs.readFile(dataPath, 'utf8');
        let dataName = JSON.parse(data)
        ...
        if (index !== -1) {
            // Actualizar los datos del restaurante
            dataName[index] = { ...dataName[index], status };
